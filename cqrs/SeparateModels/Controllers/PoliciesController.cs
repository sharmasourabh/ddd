...
namespace SeparateModels.Controllers
{
  ...
  public class PoliciesController : ControllerBase
  {
    ...
    public async Task<IActionResult> GetPolicyVersionsList([FromRoute] string policyNumber)
    {
    ...
    public async Task<IActionResult> GetPolicyVersionDetails([FromRoute] string policyNumber, [FromRoute] int versionNumber)
    {
      var result = await mediator.Send(new GetPolicyVersionDetailsQuery { PolicyNumber = policyNumber, VersionNumber = versionNumber });
      return Ok(result);
    }

    // Query message is a parameter
    [HttpPost("find")]
